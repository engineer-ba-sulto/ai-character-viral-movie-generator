...
- [ ] T001 [P] `src/app/character-animation/` ディレクトリを作成する。
- [ ] T002 [P] `src/components/` ディレクトリが存在することを確認する (なければ作成)。
- [ ] T003 [P] `src/utils/` ディレクトリを作成する。
- [ ] T004 `bun add @google/genai` を実行して、必要な依存関係をインストールする。
