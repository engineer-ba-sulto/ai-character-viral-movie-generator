...
const SavedCharacters: React.FC<SavedCharactersProps> = ({ characters, selectedId, onSelect, onPreview }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 md:p-8">
      <h2 className="text-xl font-bold text-banana-dark mb-4">あなたのキャラクター</h2>
        ...
        {characters.map((char) => (
          <div
            ...
            className={`group relative cursor-pointer rounded-lg overflow-hidden border-4 transition-all duration-200 ${
              selectedId === char.id ? 'border-banana-yellow' : 'border-transparent hover:border-banana-yellow/50'
            }`}
            aria-label={`キャラクターを選択: ${char.description.substring(0, 30)}`}
          >
            <img
